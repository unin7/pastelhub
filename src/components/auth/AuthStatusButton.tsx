...
export function AuthStatusButton({ status, onLogin, onVerify }: AuthStatusButtonProps) {
  ...
  if (status === 'guest') {
    return (
      <button
        onClick={onLogin}
        className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs transition-all hover:scale-105 active:scale-95"
      >
  ...
  if (status === 'google') {
    return (
      <button
        onClick={onVerify}
        className="flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 font-bold text-xs transition-all hover:scale-105 active:scale-95 shadow-sm shadow-blue-100"
      >
