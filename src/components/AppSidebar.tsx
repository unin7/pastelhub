...
import { Radio, Search, Calendar, FileText, Twitter as TwitterIcon, Radio as RadioIcon, Music, Youtube, Image, Mic, Gamepad, ShoppingBag, Megaphone, Bell, Ticket, Disc, Store, Calculator, CheckSquare, Trophy, TrendingUp, Wrench, BookOpen, Heart, PenTool, Users, Siren } from "lucide-react";
...
function HomeSidebarContent() {
  const { data: members } = useJsonData<any[]>('status');
  return (
    <div className="h-full px-3 py-4">
      <div className="mb-4 px-2 flex items-center gap-2"><Radio className="w-4 h-4 text-purple-600 animate-pulse" /><p className="text-sm font-bold text-purple-900/80 tracking-wide">LIVE STATION</p></div>
           ...
           return (
             <a key={idx} href={member.liveUrl} target="_blank" rel="noreferrer" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-300 cursor-pointer group border ${isLive ? 'bg-white shadow-sm border-purple-100 hover:shadow-md hover:border-purple-200' : 'border-transparent hover:bg-white/60 hover:border-gray-100'}`}>
               ...
               <div className="flex-1 min-w-0">
                 <div className="flex items-center justify-between"><span className={`text-sm font-bold truncate ${isLive ? 'text-gray-900' : 'text-gray-500 group-hover:text-gray-700'}`}>{member.name}</span>{isLive && <span className="text-[10px] font-extrabold text-red-500 bg-red-50 px-1.5 py-0.5 rounded-full animate-pulse tracking-tight">LIVE</span>}</div>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-4 sticky top-0 bg-white/50 backdrop-blur-sm z-10 border-b border-indigo-100/50">
        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-400" /><input type="text" placeholder="가이드 검색..." className="w-full pl-9 pr-4 py-2 bg-white/80 border border-indigo-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all shadow-sm" /></div>
      </div>
      <div className="flex-1 overflow-y-auto px-2 py-3">
        <SidebarGroupLabel className="px-4 text-xs font-bold text-indigo-400/80 mb-2">목차</SidebarGroupLabel>
            ...
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton asChild className="h-10 rounded-lg hover:bg-indigo-50/80 hover:text-indigo-600 text-gray-500 transition-all">
                <Link to={item.url} onClick={() => setOpenMobile(false)} className="flex items-center gap-3 px-3">
                  <div className="p-1.5 rounded-md bg-indigo-50 text-indigo-400 group-hover:bg-white group-hover:shadow-sm transition-all"><item.icon className="size-4" /></div><span className="font-medium text-sm">{item.title}</span>
                </Link>
              </SidebarMenuButton>
            </SidebarMenuItem>
          ))}
...
export function AppSidebar() {
  ...
  let items: any[] = [];
  let label = "";
  let themeConfig = { activeBg: "", iconActive: "", iconInactive: "", textActive: "", textInactive: "", hover: "" };

  if (pathname.startsWith("/news")) { items = newsSections; label = "소식 (News)"; themeConfig = { activeBg: "bg-white/90 border-blue-200 shadow-lg ring-1 ring-blue-100", iconActive: "bg-gradient-to-br from-blue-400 to-cyan-400 shadow-blue-200 text-white", iconInactive: "bg-white text-blue-300 group-hover:text-blue-400", textActive: "text-blue-900", textInactive: "text-gray-500", hover: "hover:bg-white/80 hover:border-blue-100 hover:shadow-md" }; }
  else if (pathname.startsWith("/goods")) { items = goodsSections; label = "굿즈 · 멤버십"; themeConfig = { activeBg: "bg-white/90 border-purple-200 shadow-lg ring-1 ring-purple-100", iconActive: "bg-gradient-to-br from-purple-400 to-violet-400 shadow-purple-200 text-white", iconInactive: "bg-white text-purple-300 group-hover:text-purple-400", textActive: "text-purple-900", textInactive: "text-gray-500", hover: "hover:bg-white/80 hover:border-purple-100 hover:shadow-md" }; }
  else if (pathname.startsWith("/activities")) { items = activitySections; label = "활동 (Activities)"; themeConfig = { activeBg: "bg-white/90 border-pink-200 shadow-lg ring-1 ring-pink-100", iconActive: "bg-gradient-to-br from-pink-400 to-rose-400 shadow-pink-200 text-white", iconInactive: "bg-white text-pink-300 group-hover:text-pink-400", textActive: "text-pink-900", textInactive: "text-gray-500", hover: "hover:bg-white/80 hover:border-pink-100 hover:shadow-md" }; }
  else if (pathname.startsWith("/others")) { items = otherSections; label = "기타 (Others)"; themeConfig = { activeBg: "bg-white/90 border-emerald-200 shadow-lg ring-1 ring-emerald-100", iconActive: "bg-gradient-to-br from-emerald-400 to-teal-400 shadow-emerald-200 text-white", iconInactive: "bg-white text-emerald-300 group-hover:text-emerald-400", textActive: "text-emerald-900", textInactive: "text-gray-500", hover: "hover:bg-white/80 hover:border-emerald-100 hover:shadow-md" }; }

  return (
    <Sidebar collapsible="none" className="border-r bg-white/30 backdrop-blur-sm w-72 h-full">
      <SidebarContent className="px-2 pt-6">
        {pathname === "/" ? <HomeSidebarContent /> : pathname.startsWith("/guide") ? <GuideSidebarContent /> : (
          <SidebarGroup>
            <SidebarGroupLabel className="text-xs font-bold text-gray-400 uppercase tracking-wider px-3 mb-1">{label}</SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu className="gap-2">
                {items.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild className={cn("h-12 rounded-xl transition-all duration-300 border border-transparent font-medium group", pathname === item.url ? themeConfig.activeBg : themeConfig.hover, pathname === item.url ? "scale-[1.02]" : "hover:scale-[1.01]")}>
                      <Link to={item.url} onClick={() => setOpenMobile(false)} className="flex items-center gap-3 px-3">
                        <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center transition-transform duration-300 group-hover:scale-110", pathname === item.url ? themeConfig.iconActive : themeConfig.iconInactive)}><item.icon className="size-4" /></div>
                        <span className={cn("text-sm transition-colors", pathname === item.url ? themeConfig.textActive : themeConfig.textInactive, !pathname.startsWith(item.url) && "group-hover:text-gray-700")}>{item.title}</span>
                      </Link>
                    </SidebarMenuButton>
                  </SidebarMenuItem>
                ))}
